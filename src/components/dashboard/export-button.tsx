...
export interface ExportButtonProps {
  /** Whether the user can export (STUDIO/ATELIER plans) */
  canExport: boolean;
 ...
 * STUDIO/ATELIER users can select a period and download the file.
 * ESSENTIEL users see a disabled button with upgrade tooltip.
 *
 * @example
 * ```tsx
 * <ExportButton
 ...
 * ```
 */
export function ExportButton({ canExport, creatorId, className }: ExportButtonProps) {
  ...
  }, [canExport, creatorId, selectedPeriod]);

  const periods: ExportPeriod[] = ['today', 'week', 'month', 'quarter', 'year'];

  // Disabled state for ESSENTIEL users
  if (!canExport) {
    return (
      <div className={cn('relative group', className)}>
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">
          Disponible avec Studio ou Atelier
